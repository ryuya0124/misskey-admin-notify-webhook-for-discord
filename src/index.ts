...
export default {
	async fetch(request: Request, env: { SECRET: string | null; DISCORD: string | null }) {
		// ▼▼▼ 追加する部分 ▼▼▼
		const headerSecret = request.headers.get('X-Misskey-Hook-Secret');
		console.log(`[DEBUG] Env Secret   : '${env.SECRET}'`); // Cloudflareの設定値
		console.log(`[DEBUG] Header Secret: '${headerSecret}'`); // Misskeyから送られてきた値
		console.log(`[DEBUG] Match?       : ${headerSecret === env.SECRET}`);
		// ▲▲▲ 追加ここまで ▲▲▲
